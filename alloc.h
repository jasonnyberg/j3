...
private:
    ...
    T *getPtr() const { return getAllocator().getPtr(slabIndex, slabOffset); }
...
public:
    ...
    CompressedPointer(T *ptr) { *this = fromRawPtr(ptr); }

    T &operator*() const { return *(T *)*this; }
    T *operator->() const { return (T *)*this; }

    operator bool() const { return !(slabIndex == 0 && slabOffset == 0); }
    bool valid() const { return getAllocator().valid(slabIndex, slabOffset); }

    static CompressedPointer<T> allocate() {
    ...
    static CompressedPointer<T> fromRawPtr(T *ptr) {
...
public:
    ...
    std::pair<uint16_t, uint16_t> allocate() {
        for (size_t i = 0; i < NUM_SLABS; ++i) {
            if (slabs[i] && slabs[i]->count == SLAB_SIZE) {
                continue;
            }
            if (slabs[i]) {
                size_t j = SLAB_SIZE;
